...
void RadarMain::Init()
{
	...
	vertex[0].Position = D3DXVECTOR3(-100.0f, -100.0f, 0.0f);			//座標
	vertex[0].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			//法線ベクトル
	vertex[0].Diffuse = COLOR_WHITE;	//マテリアルカラー
...
void RadarMain::Draw()
{
	...
	vertex[0].Position = D3DXVECTOR3(m_Position.x - 100.0f, m_Position.y - 100.0f, m_Position.z);			//座標
	vertex[0].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			//法線ベクトル
	vertex[0].Diffuse = COLOR_WHITE;
	vertex[0].TexCoord = D3DXVECTOR2(0.0f, 0.0f);				//テクスチャ座標

	vertex[1].Position = D3DXVECTOR3(m_Position.x + 100.0f, m_Position.y - 100.0f, m_Position.z);
	vertex[1].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	vertex[1].Diffuse = COLOR_WHITE;
	vertex[1].TexCoord = D3DXVECTOR2(1.0f, 0.0f);

	vertex[2].Position = D3DXVECTOR3(m_Position.x - 100.0f, m_Position.y + 100.0f, m_Position.z);
	vertex[2].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	vertex[2].Diffuse = COLOR_WHITE;
	vertex[2].TexCoord = D3DXVECTOR2(0.0f, 1.0f);

	vertex[3].Position = D3DXVECTOR3(m_Position.x + 100.0f, m_Position.y + 100.0f, m_Position.z);
	vertex[3].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	vertex[3].Diffuse = COLOR_WHITE;
	vertex[3].TexCoord = D3DXVECTOR2(1.0f, 1.0f);
