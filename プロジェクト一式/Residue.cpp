...
void Residue::Init()
{
	m_Position = D3DXVECTOR3(TEX_WIDTH + SCORE_WIDTH * MAX_HP + TEX_WIDTH + SCORE_WIDTH * MAX_COIN_DIGIT, SCORE_HEIGHT, 0.0f);
...
void Residue::Update()
{
	...
	{
		m_Position = m_Position = D3DXVECTOR3(SCREEN_WIDTH / 2 + SCORE_WIDTH, SCREEN_HEIGHT / 2 + ADJUSTMENT_SCORE_HEIGHT, 0.0f);
	}
}
...
void Residue::Draw()
{
	...
	do
	{
		score /= 10;
		digit += 1;
		if (digit > MAX_SCORE_DIGIT)
		{
			digit = MAX_SCORE_DIGIT;
		}
