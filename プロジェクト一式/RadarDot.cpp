...
void RadarDot::Init()
{
	...
	vertex[0].Position = D3DXVECTOR3(-5.0f, -5.0f, 0.0f);		//座標
	vertex[0].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			//法線ベクトル
	vertex[0].Diffuse = D3DXVECTOR4(1.0f, 0.0f, 0.0f, 1.0f);	//マテリアルカラー
...
void RadarDot::Draw()
{
	if (m_isShow == true)
	{
		...
		vertex[0].Position = D3DXVECTOR3(m_Position.x - 5.0f, m_Position.y - 5.0f, m_Position.z);			//座標
		vertex[0].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);			//法線ベクトル
		vertex[0].Diffuse = m_MyColor;
		vertex[0].TexCoord = D3DXVECTOR2(0.0f, 0.0f);				//テクスチャ座標

		vertex[1].Position = D3DXVECTOR3(m_Position.x + 5.0f, m_Position.y - 5.0f, m_Position.z);
		vertex[1].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		vertex[1].Diffuse = m_MyColor;
		vertex[1].TexCoord = D3DXVECTOR2(1.0f, 0.0f);

		vertex[2].Position = D3DXVECTOR3(m_Position.x - 5.0f, m_Position.y + 5.0f, m_Position.z);
		vertex[2].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		vertex[2].Diffuse = m_MyColor;
		vertex[2].TexCoord = D3DXVECTOR2(0.0f, 1.0f);

		vertex[3].Position = D3DXVECTOR3(m_Position.x + 5.0f, m_Position.y + 5.0f, m_Position.z);
		vertex[3].Normal = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
		vertex[3].Diffuse = m_MyColor;
		vertex[3].TexCoord = D3DXVECTOR2(1.0f, 1.0f);
